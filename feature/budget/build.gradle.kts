...
kotlin {
    android {
        namespace = "com.georgeci.moneysurfer.feature.budget"
    }
...
// Pin the generated `Res` class package so callers can import `moneysurfer.feature.budget.generated.resources.*`
// regardless of where compose-multiplatform's default would land it for nested gradle paths.
compose.resources {
    packageOfResClass = "moneysurfer.feature.budget.generated.resources"
}

koinCompiler {
    // Per-module verification fails because budget depends on data/domain/sync/shared bindings
