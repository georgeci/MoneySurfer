...
kotlin {
    android {
        namespace = "com.georgeci.moneysurfer.feature.insights"
    }
...
// Pin the generated `Res` class package so callers can import
// `moneysurfer.feature.insights.generated.resources.*` regardless of where compose-multiplatform's
// default would land it for nested gradle paths.
compose.resources {
    packageOfResClass = "moneysurfer.feature.insights.generated.resources"
}

koinCompiler {
    // Per-module verification fails because insights depends on data/domain bindings that aren't
    // visible from this module. Full-graph verification runs at the :composeApp module instead
    // (AppModule includes every sub-module).
    compileSafety = false
}
