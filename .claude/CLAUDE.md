...
- **Never force push** — `git push --force`, `-f`, `--force-with-lease`, `--force-if-includes` and `+refspec` are forbidden under any circumstances, on any branch, including after a rebase or a failed push. Blocked by a `PreToolUse` hook and `permissions.deny` in [.claude/settings.json](.claude/settings.json), and by the git [`.githooks/pre-push`](.githooks/pre-push) hook (`git config core.hooksPath .githooks` — one-time, per clone). Never pass `--no-verify`. If history really must be rewritten, stop and ask the user to do it manually.
