 ...
 *
 * **Only monthly caps count.** [slices] covers [window]; a weekly or yearly budget's amount is not
 * a figure for that window, and dividing one into a pro-rata limit would invent a cap the user
 * never set.
 *
 * **The cap's own window is not this one.** Budget windows are anchored on `Budget.startDate`, so a
 * cap created mid-month runs 15th-to-15th while this breakdown runs the calendar month. Every
 * number *here* is measured over [window] — spend, limit and status alike, so the widget is
 * internally consistent — but a status shown here can differ from the same budget's status on the
 * Budgets screen, which reads the budget's own window and includes its rollover carry.
 */
...
private fun capFor(slice: CategorySpendSlice, budgets: List<Budget>): CategoryCap? {
    ...
    if (budget.period != BudgetPeriod.MONTHLY) return null
    return CategoryCap(
        limit = budget.amount,
